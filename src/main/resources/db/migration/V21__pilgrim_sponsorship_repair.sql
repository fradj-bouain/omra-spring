-- Réparation idempotente : si la base n'a pas reçu V20 (ou schéma partiel), on ajoute ce qui manque.
-- Ne pas dupliquer la logique métier : uniquement colonnes / index / tables attendues par l'application.
...
ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS referrer_pilgrim_id BIGINT;
ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS referral_points INTEGER NOT NULL DEFAULT 0;

-- FK vers pilgrims (si la colonne existait sans contrainte)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'pilgrims_referrer_pilgrim_id_fkey'
  ) THEN
    ALTER TABLE pilgrims
      ADD CONSTRAINT pilgrims_referrer_pilgrim_id_fkey
      FOREIGN KEY (referrer_pilgrim_id) REFERENCES pilgrims(id) ON DELETE SET NULL;
  END IF;
EXCEPTION
